import "./exploreIndiaSlider.css";

import Slider from "react-slick";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import { ExploreIndiaArray } from "../../Constant/constant";
import { Link, useNavigate } from "react-router-dom";
import { useRef } from "react";

function ExploreIndiaSlider() {
  const settings = {
    dots: false,
    infinite: true,
    speed: 500,
  ...
  return (
    <div className="propertySliderContainer">
      <div className="propertySliderText">Explore India</div>
      <div className="exploreIndiaSliderLightText">
      ...
      <div className="propertySlider">
        ...
        <Slider {...settings} ref={sliderRef}>
          {ExploreIndiaArray.map((val, index) => {
            return (
              <div className="SliderCard" key={index}>
                <Link to={`/hotel-list/${val.location}`}>
                  <div>
                    <img src={val.image} alt="" />
                    <div className="sliderCardDestination">{val.name}</div>
                    <div className="sliderCardProperty">{val.text}</div>
                  </div>
                </Link>
              </div>
            );
...
export default ExploreIndiaSlider;
